...
static inline _byte_t* fill_n_vec(type_t* type, _byte_t* destPosition, size_t n, void* val)
{
  bfun_t cpyctor = type->_t_typecopy;
...
{
  return abs(((int32_t) (last->_t_pos - first->_t_pos)));
}
...
static inline do_insert_value(vector_t* pvec, _byte_t* dest, void* value)
{
  assert(dest >= pvec->_pby_start);
  assert(dest <= pvec->_pby_finish);
