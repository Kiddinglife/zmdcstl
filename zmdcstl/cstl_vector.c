...
bool vector_is_inited(const vector_t* cpvec_vector)
{
  ...
  if (_GET_VECTOR_TYPE_INFO_TYPE(cpvec_vector) == NULL)
    return false;
  if (cpvec_vector->_pby_start == NULL
  ...
  if (cpvec_vector->_pby_start != NULL
      && cpvec_vector->_pby_finish != NULL
      && cpvec_vector->_pby_endofstorage != NULL
      && cpvec_vector->_pby_finish
          > cpvec_vector->_pby_start
      && cpvec_vector->_pby_endofstorage
          > cpvec_vector->_pby_start
      && cpvec_vector->_pby_endofstorage
          >= cpvec_vector->_pby_finish)
    return true;
...
bool vector_iterator_valid(const vector_t* cpvec_vector,
    vector_iterator_t* it_iter)
{
  return vector_is_inited(cpvec_vector)
      &&
      _VECTOR_ITERATOR_CONTAINER_TYPE(it_iter)
          == _VECTOR_CONTAINER &&
...
bool vector_iterator_valid_end(const vector_t* cpvec_vector,
    vector_iterator_t* it_iter)
{
  return vector_is_inited(cpvec_vector)
      &&
...
{
  assert(
      vector_iterator_valid(_VECTOR_ITERATOR_CONTAINER(it_iter), it_iter));
  _VECTOR_ITERATOR_COREPOS(it_iter) +=
      _GET_VECTOR_TYPE_SIZE(
          _VECTOR_ITERATOR_CONTAINER(it_iter));
  assert(
      vector_iterator_valid(_VECTOR_ITERATOR_CONTAINER(it_iter), it_iter));
}

void vector_end(vector_t* cpvec_vector,
    vector_iterator_t* it_end)
{
  assert(vector_is_inited(cpvec_vector));
  _ITERATOR_CONTAINER(it_end) = cpvec_vector;
  _VECTOR_ITERATOR_COREPOS(it_end) =
      cpvec_vector->_pby_finish;
}

void vector_begin(vector_t* cpvec_vector,
    vector_iterator_t* begin)
{
  assert(vector_is_inited(cpvec_vector));
  _ITERATOR_CONTAINER(begin) = cpvec_vector;
  _VECTOR_ITERATOR_COREPOS(begin) =
      cpvec_vector->_pby_start;
}
...
void vector_end_again(vector_iterator_t* it_end)
{
  ...
  assert(
      vector_is_inited((vector_t* )it_end->_pt_container));
  _VECTOR_ITERATOR_COREPOS(it_end) =
      ((vector_t*) it_end->_pt_container)->_pby_finish;
}
...
bool vector_iterator_equal(vector_iterator_t* it_first,
    vector_iterator_t* it_second)
{
  assert(iterator_same_type(it_first, it_second));
  assert(
      _VECTOR_ITERATOR_CONTAINER(it_first) == _VECTOR_ITERATOR_CONTAINER(it_second));
  assert(
      vector_iterator_valid(_VECTOR_ITERATOR_CONTAINER(it_first), it_first));
  assert(
      vector_iterator_valid(_VECTOR_ITERATOR_CONTAINER(it_second), it_second));
  return _VECTOR_ITERATOR_COREPOS(it_first)
      == _VECTOR_ITERATOR_COREPOS(it_second);
}

void vector_iterator_get_value(vector_iterator_t* it_iter,
    void* pv_value)
{
  ...
  assert(
      vector_iterator_valid(_VECTOR_ITERATOR_CONTAINER(it_iter), it_iter));
  assert(
      (void*)it_iter->_t_pos != _VECTOR_ITERATOR_CONTAINER(it_iter)->_pby_finish);
  size_t size = 0;
  ...
  //else
  {
    size = _GET_VECTOR_TYPE_SIZE(
        _VECTOR_ITERATOR_CONTAINER(it_iter));
    _GET_VECTOR_TYPE_COPY_FUNCTION(_VECTOR_ITERATOR_CONTAINER(it_iter))(
        pv_value, _VECTOR_ITERATOR_COREPOS(it_iter), &size);
    assert(size);
  }
}

bool vector_iterator_less(vector_iterator_t* it_first,
    vector_iterator_t* it_second)
{
  assert(iterator_same_type(it_first, it_second));
  assert(
...
bool vector_iterator_before(vector_iterator_t* it_first,
    vector_iterator_t* it_second)
{
  return vector_iterator_less(it_first, it_second);
}

void vector_iterator_set_value(vector_iterator_t* it_iter,
    const void* cpv_value)
{
  ...
  assert(
      vector_iterator_valid(_VECTOR_ITERATOR_CONTAINER(it_iter), it_iter));
  assert(
      (_byte_t*)it_iter->_t_pos < _VECTOR_ITERATOR_CONTAINER(it_iter)->_pby_finish);
  ...
  //else
  //{
  b_result = _GET_VECTOR_TYPE_SIZE(
      _VECTOR_ITERATOR_CONTAINER(it_iter));
  _GET_VECTOR_TYPE_COPY_FUNCTION(_VECTOR_ITERATOR_CONTAINER(it_iter))(
      _VECTOR_ITERATOR_COREPOS(it_iter), cpv_value,
      &b_result);
  assert(b_result);
  //}
}
...
void vector_iterator_next_n(vector_iterator_t* it_iter,
    size_t n_step)
{
  assert(
      vector_iterator_valid(_VECTOR_ITERATOR_CONTAINER(it_iter), it_iter));
  _VECTOR_ITERATOR_COREPOS(it_iter) +=
  _GET_VECTOR_TYPE_SIZE(_VECTOR_ITERATOR_CONTAINER(it_iter))
      * n_step;
  assert(
      vector_iterator_valid(_VECTOR_ITERATOR_CONTAINER(it_iter), it_iter));
}

void vector_iterator_prev_n(vector_iterator_t* it_iter,
    size_t n_step)
{
  assert(
      vector_iterator_valid(_VECTOR_ITERATOR_CONTAINER(it_iter), it_iter));
...
void* vector_iterator_at(vector_iterator_t* it_iter,
    size_t n_index)
{
...
void* vector_iterator_get_pointer(
    vector_iterator_t* it_iter)
{
  assert(
      vector_iterator_valid(_VECTOR_ITERATOR_CONTAINER(it_iter), it_iter));
...
void* vector_iterator_get_pointer_ignore_cstr(
    vector_iterator_t* it_iter)
{
  assert(
      vector_iterator_valid(_VECTOR_ITERATOR_CONTAINER(it_iter), it_iter));
...
size_t vector_iterator_minus(vector_iterator_t* it_first,
    vector_iterator_t* it_second)
{
  assert(iterator_same_type(it_first, it_second));
  assert(
      _VECTOR_ITERATOR_CONTAINER(it_first) == _VECTOR_ITERATOR_CONTAINER(it_second));
  assert(
      vector_iterator_valid_end(_VECTOR_ITERATOR_CONTAINER(it_first), it_first));
  assert(
      vector_iterator_valid_end(_VECTOR_ITERATOR_CONTAINER(it_second), it_second));
  return (_VECTOR_ITERATOR_COREPOS(it_second)
      - _VECTOR_ITERATOR_COREPOS(it_first))
      / (int) _GET_VECTOR_TYPE_SIZE(
          _VECTOR_ITERATOR_CONTAINER(it_first));
}
...
void vector_ctor(vector_t* pvec_vector, size_t size, ...)
{
  ...
  {
    pvec_vector->meta._t_typeinfo.typeids_ptr = cstl_alloc(
        unsigned char, size);
    pvec_vector->_pby_endofstorage =
        pvec_vector->meta._t_typeinfo.typeids_ptr;
  } else
  {
    pvec_vector->_pby_endofstorage =
        (unsigned char*) &pvec_vector->meta._t_typeinfo.typeids_ptr;
  }
...
void vector_ctor_n(vector_t* pvec_vector, size_t elesize,
    size_t size, ...)
{
  if (size > TYPE_ID_SIZE)
  {
  ...
  if (type->_t_typeinit)
  {
    ...
    pvec_vector->_pby_finish = pvec_vector->_pby_start =
        cstl_alloc_ex_totaln(type->_t_typealign,
            elesize * size);
    for (; elesize > 0;
        elesize--, pvec_vector->_pby_finish += size)
      type->_t_typeinit(pvec_vector->_pby_finish, &ret);
  } else
  {
    size = type->_t_typesize * elesize;
    pvec_vector->_pby_finish = pvec_vector->_pby_start =
        cstl_alloc_ex_totaln(type->_t_typealign, size);
...
void vector_ctor_n_v(vector_t* pvec_vector, size_t elesize,
    void* val, size_t size, ...)
{
  ...
  type_t* type = _GET_VECTOR_TYPE_INFO_TYPE(pvec_vector);
  size = type->_t_typesize;
  pvec_vector->_pby_start = pvec_vector->_pby_finish =
      cstl_alloc_ex(type->_t_typesize, type->_t_typealign,
          elesize);
  ...
  switch (type->_t_typeid)
  {
    ...
    case cstl_uint16:
      for (; elesize > 0;
          elesize--, pvec_vector->_pby_finish += size)
        *(unsigned short*) pvec_vector->_pby_finish =
            *(unsigned short*) val;
      break;
    case cstl_int32:
      for (; elesize > 0;
    ...
    case cstl_uint32:
      for (; elesize > 0;
          elesize--, pvec_vector->_pby_finish += size)
        *(unsigned int*) pvec_vector->_pby_finish =
            *(unsigned int*) val;
      break;
    case cstl_int64:
      for (; elesize > 0;
          elesize--, pvec_vector->_pby_finish += size)
        *(long long*) pvec_vector->_pby_finish =
            *(long long*) val;
      break;
    case cstl_uint64:
      for (; elesize > 0;
          elesize--, pvec_vector->_pby_finish += size)
        *(unsigned long long*) pvec_vector->_pby_finish =
    ...
    default:
      if (type->_t_typecopy)
      {
        ...
        for (; elesize > 0;
            elesize--, pvec_vector->_pby_finish += size)
          type->_t_typecopy(pvec_vector->_pby_finish, val,
              &ret);
      } else
      {
        for (; elesize > 0;
...
void vector_ctor_range(vector_t* pvec_vector,
    forward_iterator_t* first, forward_iterator_t* last)
{
  ...
  {
    type_t* type = _GET_VECTOR_TYPE_INFO_TYPE(pvec_vector);
    pvec_vector->_pby_start = cstl_alloc_ex(
        type->_t_typesize, type->_t_typealign, size);
    ...
    pvec_vector->_pby_endofstorage =
        pvec_vector->_pby_finish = ritr._t_pos;
  }
}

void vector_ctor_range_n(vector_t* pvec_vector,
    forward_iterator_t* first, size_t size)
{
  pvec_vector->meta = *_ITERATOR_META_TYPE(first);
  // copy x's type info
  if (pvec_vector->meta._t_typeinfo._t_typeidsize
      > TYPE_ID_SIZE)
  {
    pvec_vector->meta._t_typeinfo.typeids_ptr = cstl_alloc(
        unsigned char,
    ...
    cstl_memcpy(pvec_vector->meta._t_typeinfo.typeids_ptr,
    _ITERATOR_TYPE_INFO(first).typeids_ptr,
        pvec_vector->meta._t_typeinfo._t_typeidsize);
  }
  if (size == 0)
  {
...
void vector_ctor_vector(vector_t* pvec_vector, vector_t* x)
{
  ...
  if (size > TYPE_ID_SIZE)
  {
    pvec_vector->meta._t_typeinfo.typeids_ptr = cstl_alloc(
        unsigned char, size);
    cstl_memcpy(pvec_vector->meta._t_typeinfo.typeids_ptr,
        x->meta._t_typeinfo.typeids_ptr, size);
  }
  ...
  } else
  {
    ...
    if (type->_t_typecopy)
    {
      ...
      size = type->_t_typesize;
      for (; from != end;
          from += size, pvec_vector->_pby_finish += size)
        type->_t_typecopy(pvec_vector->_pby_finish, from,
            &ret);
    } else
    {
...
{
  return pvec->_pby_start
      + position
          * _GET_VECTOR_TYPE_INFO_TYPE(pvec)->_t_typesize;
}
void vector_resize_n_v(vector_t* cpvec_vector, size_t n,
    const void* value)
{
...
void vector_reserve(vector_t* cpvec_vector, size_t n)
{
  ...
  {
    // do grow
    _byte_t* pNewData = cstl_alloc_ex_totaln(
        type->_t_typealign, size);
    ...
    cpvec_vector->_pby_endofstorage = pNewStart
        + totalbytes;
  }
}
...
static inline void do_insert_value_at_end_(
    vector_t* pvec_vector, const void* value)
{
  type_t* type = _GET_VECTOR_TYPE_INFO_TYPE(pvec_vector);
  const size_t nPrevSize = pvec_vector->_pby_finish
      - pvec_vector->_pby_start;
  const size_t nNewSize = get_new_capacity_(nPrevSize,
      type->_t_typesize);
  _byte_t* pNewData = cstl_alloc_ex_totaln(
      type->_t_typealign, nNewSize);
  ...
  if (type->_t_typecopy)
  {
    ...
    for (;
        pvec_vector->_pby_start != pvec_vector->_pby_finish;
        pvec_vector->_pby_start += type->_t_typesize)
    {
  ...
  {
    // copy the old elements to pNewData
    cstl_memcpy(pNewEnd, pvec_vector->_pby_start,
        nPrevSize);
    pNewEnd += nPrevSize;
    // copy new element to pNewEnd
...
bool vector_is_same_type(const vector_t* cpvec_first,
    const vector_t* cpvec_second)
{
  ...
  type_t* typefirst = _GET_VECTOR_TYPE_INFO_TYPE(
      cpvec_first);
  type_t* typesecond = _GET_VECTOR_TYPE_INFO_TYPE(
      cpvec_second);
  type_info_t* typeinfofirst = &_GET_VECTOR_TYPE_INFO(
      cpvec_first);
  type_info_t* typeinfosecond = &_GET_VECTOR_TYPE_INFO(
      cpvec_second);
  return (typefirst->_t_typeid == typesecond->_t_typeid)
      && type_info_is_same(typeinfofirst, typeinfosecond);
}

size_t vector_size(vector_t* cpvec_vector)
{
  assert(vector_is_inited(cpvec_vector));
  return (cpvec_vector->_pby_finish
      - cpvec_vector->_pby_start)
      / _GET_VECTOR_TYPE_SIZE(cpvec_vector);
}
bool empty(vector_t* cpvec_vector)
{
  return cpvec_vector->_pby_start
      == cpvec_vector->_pby_finish;
}
size_t vector_capacity(vector_t* cpvec_vector)
{
  assert(vector_is_inited(cpvec_vector));
  return (cpvec_vector->_pby_endofstorage
      - cpvec_vector->_pby_start)
      / _GET_VECTOR_TYPE_SIZE(cpvec_vector);
}

extern bool equal_from_continus(_byte_t* first1,
    _byte_t* last1, input_iterator_t* first2);
bool vector_equal(vector_t* cpvec_first,
    vector_t* cpvec_second)
{
  assert(vector_is_inited(cpvec_first));
  assert(vector_is_inited(cpvec_second));
  ...
  return equal_from_continus(cpvec_first->_pby_start,
      cpvec_first->_pby_finish, &f2);
}

void vector_swap(vector_t* pvec_first,
...
bool destruct_element)
{
  assert(
      position->_t_pos
          >= ((vector_t* )(position->_pt_container))->_pby_start);
  assert(
      position->_t_pos
          < ((vector_t* )(position->_pt_container))->_pby_finish);

  type_t* type = _GET_VECTOR_TYPE_INFO_TYPE(
      position->_pt_container);
  size_t tsize = type->_t_typesize;
  ...
  {
    case cstl_int8:
      for (; first != last; first += tsize, destPosition +=
          tsize)
    ...
    case cstl_uint64:
      for (; first != last; first += tsize, destPosition +=
          tsize)
        *(unsigned long long*) destPosition =
...
void vector_assign_n_v(vector_t* pvec, const void* val,
    size_t elesize)
{
      ...
      > pvec->_pby_endofstorage - pvec->_pby_start)
  {
    if (pvec->_pby_start != NULL)
    {
      ...
      _byte_t* ss = s;
      pvec->_pby_start = pvec->_pby_finish =
          cstl_alloc_ex_totaln(type->_t_typealign,
              totalbytes);

      if (type->_t_typecopy)
      {
        for (; elesize > 0; elesize--, pvec->_pby_finish +=
            tsize)
        {
          if (s != e)
          {
      ...
      {
        case cstl_int8:
          for (; elesize > 0; elesize--, destPosition +=
              tsize)
      ...
      > pvec->_pby_finish - pvec->_pby_start)
  {
    ...
    uninitialized_fill_n(&ritr, val,
        elesize - vector_size(pvec));
...
void vector_assign_range(input_iterator_t * first,
    input_iterator_t * last)
{
  //todo  need earse() and insert() done
}

void vector_assign_range_n(input_iterator_t * first,
    size_t n)
{
  //todo  need earse() and insert() done
}
