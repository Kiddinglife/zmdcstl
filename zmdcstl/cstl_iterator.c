...
bool iterator_limit_type(iterator_t* it_iter, iteratortype_t t_limittype)
{
  ...
  case _INPUT_ITERATOR:
    if (it_iter->_t_iteratortype >= _INPUT_ITERATOR)
      return true;
    break;
  case _OUTPUT_ITERATOR:
    if (it_iter->_t_iteratortype >= _OUTPUT_ITERATOR)
      return true;
    break;
  case _FORWARD_ITERATOR:
    if (it_iter->_t_iteratortype >= _FORWARD_ITERATOR)
      return true;
    break;
  case _BIDIRECTIONAL_ITERATOR:
    if (it_iter->_t_iteratortype >= _BIDIRECTIONAL_ITERATOR)
      return true;
    break;
