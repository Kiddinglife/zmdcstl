...
//void (*ctor_custom_t)(va_list args, size_t size);
...
static void func_init_user_defined_type_init_destroy_copy_less(void* in)
{
  ...
  dest->b = cstl_alloc(int, 32);
  //printf("init alloc = %x\n", dest->b);
  memset(dest->b, 0, 32 * sizeof(int));
  dest->a = 32;
  memset(dest->c, 0, 32);
}
...
TEST test_vector_erase_single_element()
{
  size_t elesize = 100;
...
TEST test_vector_erase_range()
{
  ...
  ASSERT_EQ(begin._t_pos <= end._t_pos, true);
  int capacity = vector_capacity(&pvec_vector);
  vector_erase_range(&begin, &end, true);
  ASSERT_EQ(vector_size(&pvec_vector), 0);
  ASSERT_EQ(capacity, vector_capacity(&pvec_vector));
  vector_dtor(&pvec_vector);
  ...
  vector_begin(&pvec_vector, &first);
  iterator_t second;
  vector_begin(&pvec_vector, &second);
  vector_iterator_next(&second);
  capacity = vector_capacity(&pvec_vector);
  int vsize = vector_size(&pvec_vector);
  vector_erase_range(&first, &second, true);
  ASSERT_EQ(vector_size(&pvec_vector), vsize - 1);
  ASSERT_EQ(vector_size(&pvec_vector), vsize - 1);
  ASSERT_EQ(capacity, vector_capacity(&pvec_vector));
...
TEST test_vector_insert_v_copy()
{
  user_defined_type_init_destroy_copy_less firstv;
  firstv.a = 1;
  firstv.b = cstl_alloc(int, firstv.a);
...
SUITE(test_vector)
{
  ...
  RUN_TEST(test_vector_erase_single_element);
  RUN_TEST(test_vector_erase_range);
